...
import personal.wjp.sample.appcomponentfactory.initializer.InitializerB

class ComponentFactoryB:AppComponentFactory() {
    private fun log(msg:String) {
        Log.d("WJP", "ComponentFactoryB: $msg")
    }
    override fun instantiateApplicationCompat(cl: ClassLoader, className: String): Application {
        log("instantiateApplicationCompat $className")
        val app = super.instantiateApplicationCompat(cl, className)
        if (app is MyApplication) {
            app.setInitializer { InitializerB() }
        }
        return app
    }
